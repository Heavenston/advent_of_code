...
fn main() {
    ...
    loop {
        ...
        for (i, s) in scanners.iter().enumerate() {
            ...
            if let Some(gkey) = gkey {
                ...
                println!("{}[{}] -> {} {} {}", i, gkey, rs[12].round(), rs[13].round(), rs[14].round());
    ...
    let mut beacons = HashSet::<na::Vector3<i32>>::default();

    for i in 0..scanners.len() {
        let s = &scanners[i];
        let global = global_map[&i];
        for beacon in s.beacons.iter() {
            let r_pose = beacon.map(|a| a as f32);
            let g_pose = global*r_pose;
            beacons.insert(g_pose.map(|a| a.round() as i32).fixed_resize::<3, 1>(0));
        }
    }
