        ...
        where
            ...
            run wins (n:numbers) boards masks =
                ...
                case find (hasWon . snd) (zip boards newMasks) of
                  Just (wBoard, wMask) -> 
                      (uncurry $ run ((wBoard, wMask, n*getScore wBoard wMask):wins) numbers)
                      (unzip $ filter (\(a, b) -> a == wBoard && b == wMask) $ zip boards newMasks)
                  -- Just (wBoard, wMask) -> run (Just $ n * getScore wBoard wMask) numbers (delete wBoard boards) (delete wMask newMasks)
                  Nothing     -> run wins numbers boards newMasks
